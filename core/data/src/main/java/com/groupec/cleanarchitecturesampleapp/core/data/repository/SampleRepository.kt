...
import com.groupec.cleanarchitecturesampleapp.core.data.model.toRepository

import com.google.samples.apps.nowinandroid.core.database.dao.RepositoryDao
import com.google.samples.apps.nowinandroid.core.datastore.DataStoreManager
...
class SampleRepository @Inject constructor(
    ...
    /* Functions */
    override fun getAllRepositories(): Flow<List<Repository>> = flow {
        val response = api.getAllRepositories().execute()
        if (response.isSuccessful) {
           /* val repositoriesDTO = response.body() ?: emptyList()
            val repositories = repositoriesDTO.map { it.toRepository() }*/

            val repositories = (response.body() as List<NetworkRepository>).map (::toRepository)
            emit(repositories)
        } else {
